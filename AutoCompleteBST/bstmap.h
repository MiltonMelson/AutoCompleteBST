...
private:
  ...
  Node *copyHelper(Node *bstNode);

  // Creates a balanced BST out of a sorted vector.
  Node *sortedVectorToBST(vector<Node*> &v, int start, int end);

  // Deletes tree.
  void destroyTree(Node *toDel);

  // Visit traversal helpers.
  void inorderHelper(void visit(const BSTMap::value_type &item),
                     Node *curr) const;
  void preorderHelper(void visit(const BSTMap::value_type &item),
                      Node *curr) const;
  void postorderHelper(void visit(const BSTMap::value_type &item),
                       Node *curr) const;

  // Searches for values that contain prefix in BST.
  void autoCompleteSearch(Node *source, const BSTMap::key_type &prefix,
                          vector<BSTMap::value_type> &vec) const;

  Node *removeNode(const BSTMap::key_type &k, Node *subTree,
                   bool &isSuccessful);

  Node *minValueNode(Node *node);

  BSTMap::mapped_type &insertKey(Node *curr, const BSTMap::key_type &k);

  void deleteTree(Node **nodeRef);

  void treeToVector(vector<Node*> &vec, Node* currRoot);
};
